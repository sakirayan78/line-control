    ...
    var methods = {
		...
		init : function( options )
		{
						   ...
						   'insert_img'	: { "modal": true,
											...
											"onSave": function(){
											    ...
											    if ($('#imageList').data('current')) {
											        ...
											        imgurl = "";
											        var mm = $('#imageList').data('current').split(',');
											        var type = mm[0].replace('data:image/', '').replace(';base64', '');
											        var photo = new Object();

											        photo.photoType = type;
											        photo.photoasBase64 = mm[1];
											        var photo2 = new Object();
											        photo2.photo = photo;
											     
											        var request = $.ajax({
											            url: 'LineEdit/Service/ImageService.asmx/UploadImage',
											            method: "POST",
											            data: JSON.stringify(photo2),
											            dataType: "json",
			...
			var statusBar = $("<div/>", {	id : "statusbar",
	       	...
	       	methods.createContentChangedEvent($this);
		   ...
		   // var fnx = editor.bind("change", function (e) { alert(editor.html()); });
			...
			cMenuUl.append($('<li/>').append(modalTrigger))
					.append($('<li/>').append($('<a/>',{text:"Remove Image"}).click( 
						function(e) { return function(){ 
						    ...
						    var request = $.ajax({
						        url: 'LineEdit/Service/ImageService.asmx/RemoveImage',
						        method: "POST",
						        data: JSON.stringify(photo2),
						        dataType: "json",
						        contentType: "application/json; charset=utf-8",
		...
		createContentChangedEvent: function (txtarea) {
           $('.Editor-editor').bind('contentchanged', function () {
                // do something after the div content has changed
                txtarea.val($('.Editor-editor').html());
            });
		},

        contentChangedTriger:function() {
            $('.Editor-editor').trigger('contentchanged');
        }
